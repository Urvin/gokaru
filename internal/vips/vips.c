...
int vips_savejpeg_ext_go(VipsImage *in, void **buf, size_t *len,
    int quality,
    gboolean interlace,
    gboolean optimizeCoding,
    int subsampleMode,
    gboolean trellisQuant,
    gboolean overshootDeringing,
    gboolean optimizeScans,
    int quantTable
    ) {
    ...
    return vips_jpegsave_buffer(
        ...
        "trellis_quant", trellisQuant,
        "overshoot_deringing", overshootDeringing,
        "optimize_scans", optimizeScans,
...
int
vips_pngsave_go(VipsImage *in, void **buf, size_t *len, gboolean interlace, int quantize, int colors, int compression) {
  if (!quantize)
...
int
vips_webpsave_go(VipsImage *in, void **buf, size_t *len, int quality, gboolean lossless, gboolean smart_subsample, int reduction_effort) {
...
int
vips_avifsave_go(VipsImage *in, void **buf, size_t *len, int quality, int speed, gboolean lossless) {
#if VIPS_SUPPORT_AVIF
...
{
    if (vips_getpoint(in, &bg, bgn, x, y, NULL)) {
        return 1;
    }
    return 1;
}
